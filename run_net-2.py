...
import monai
from monai.handlers import CheckpointSaver, MeanDice, StatsHandler, ValidationHandler, TensorBoardStatsHandler, TensorBoardImageHandler, LrScheduleHandler, CheckpointLoader
...
#     net = monai.networks.nets.VNet(spatial_dims=3, in_channels=1, out_channels=n_classes)
...
class DiceCELoss(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.dice = monai.losses.DiceLoss(to_onehot_y=True, softmax=True, include_background=True)
        self.cross_entropy = nn.CrossEntropyLoss()
        self.focal_loss = FocalLoss(gamma=2, weight=torch.tensor([0.02, 0.98]))
    ...
    def forward(self, y_pred, y_true):
        ...
        return dice + focal_loss + cross_entropy
...
#   pre-treino:
#     images = sorted(glob.glob(os.path.join(data_folder, "images", "*.nii.gz")))
#     labels = sorted(glob.glob(os.path.join(data_folder, "masks3", "*.nii.gz")))
    
    images = sorted(glob.glob(os.path.join(data_folder, "images", "*_ct.nii.gz")))
    labels = sorted(glob.glob(os.path.join(data_folder, "masks", "*_ct.nii.gz")))
    ...
    max_epochs, lr, momentum = 300, 1e-3, 0.95
    ...
    log_dir='tensorboard_2/' + current_time
    
    val_handlers = [
        ProgressBar(),
        CheckpointSaver(save_dir=model_folder, save_dict={"net": net}, save_key_metric=True, key_metric_n_saved=3),
#         CheckpointSaver(save_dir=model_folder+'_300', save_dict={"net": net}, save_interval= 50),
        TensorBoardStatsHandler(log_dir=log_dir),
#         TensorBoardImageHandler(
#             log_dir=log_dir,
#             batch_transform=lambda batch: (batch[keys[0]], batch[keys[1]]),
#             output_transform=lambda x: (x["pred"], x["label"]),
...
#         )
    ...
    train_handlers = [
        CheckpointLoader(load_path='pre-treino/net_key_metric=0.7881.pt', load_dict={'network': net} ),
...
#     loss_function = FocalLoss(gamma=2, weight=torch.tensor([0.25, 0.75]))
...
def infer(data_folder=".", model_folder="runs", prediction_folder="output_2"):
    """
...
#     images = sorted(glob.glob(os.path.join(image_folder, "*_ct.nii.gz")))
    images = sorted(glob.glob(os.path.join(image_folder, "*_ct.nii")))
...
if __name__ == "__main__":
    ...
    """
    """
        pre-treinar-luna: python run_net-2.py train --data_folder "../../ester/samples_luna" --model_folder "pre-treino" 
        treinar: python run_net-2.py train --data_folder "../CADCOVID/COVID-19-20/COVID-19-20/Train" --model_folder "runs"
        inferencia: python run_net-2.py infer --data_folder "../CADCOVID/COVID-19-20/COVID-19-20/Validation" --model_folder "runs"
        teste: inferencia: python run_net-2.py infer --data_folder "../CADCOVID/COVID-19-20_TestSet" --model_folder "runs"
